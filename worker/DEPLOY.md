...
### 9.1 平台层（Cloudflare Dashboard / Wrangler）

```bash
wrangler deployments list
```

- 能看到最新 deployment 且状态正常。
- 时间戳与刚刚发布的时间一致。

### 9.2 健康检查（无鉴权）

```bash
curl -i https://tempure-worker.<your-subdomain>.workers.dev/health
```
...
### 9.3 鉴权链路（send-code -> verify-code -> refresh）

1) 发送验证码

```bash
curl -sS -X POST https://tempure-worker.<your-subdomain>.workers.dev/auth/send-code \
  -H 'content-type: application/json' \
  -d '{"email":"you@example.com"}'
```

`APP_ENV=dev` 时，响应应含 `debugCode`。

2) 校验验证码

```bash
curl -sS -X POST https://tempure-worker.<your-subdomain>.workers.dev/auth/verify-code \
  -H 'content-type: application/json' \
...
```bash
curl -sS -X POST https://tempure-worker.<your-subdomain>.workers.dev/auth/refresh \
  -H 'content-type: application/json' \
...
```bash
curl -sS -X POST https://tempure-worker.<your-subdomain>.workers.dev/records/batch-upsert \
  -H 'content-type: application/json' \
...
- `/health` 非 200：Worker 未成功发布或路由异常。
- `/auth/send-code` 429：触发限流，等待 5 分钟后再试。
- `/auth/verify-code` 401：验证码过期或不匹配。
- `/records/batch-upsert` 401：`Authorization` 缺失/过期。
- `/records/batch-upsert` 400：`recordDate` 不是 `YYYY-MM-DD`。
